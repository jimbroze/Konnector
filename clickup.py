...
class Clickup:
    ...
    workspace = "2193273"
    clickupEvents = [
        "taskUpdated", "taskDeleted", "taskStatusUpdated"
    ]
    events = {
    ...
    def modify_webhook(self, request):
        ...
        if self.webhookId is not None:
            return self._update_webhook(self.webhookId, self.endpoint,
                                        self.clickupEvents,
                                        list(self.lists.values()))
        else:
            return self._create_webhook(self.workspace, self.endpoint,
                                        self.clickupEvents,
                                        list(self.lists.values()))
    ...
    def check_request(self, request):
        ...
        calcHmac = hmac.new(bytes(self.webhookSecret, "utf-8"),
    ...
    def _send_request(self, location, reqType="GET", data={}):
        return helpers.send_request(
            ...
            {'Authorization': self.accessToken}, reqType, data)
    ...
    def task_received(self, data, update=False):
        ...
        if 'clickup_complete' in clickupTask:
            task['clickup_complete'] = True if clickupTask[
                'status'] == "complete" else False
        for customField in clickupTask['custom_fields']:
            if customField[
                    'id'] == self.customFieldTodoist and "value" in customField:
    ...
    def create_new_task(self, task):
        ...
        queryParams = 'custom_fields=[{"field_id":"' + self.customFieldTodoist + '","operator":"=","value":' + str(
            task['todoist_id']) + "}]"
        projectTasks = self._send_request(f"list/{listId}/task?{queryParams}",
                                          "GET", {})
        if not projectTasks:
